...
while vid.isOpened():
    ...
    edges = cv2.Canny(
        image=imagetocanny, threshold1=200, threshold2=200
    )  # detect the canny edges

    # apply hough lines
    lines = cv2.HoughLinesP(edges, 1, np.pi / 100, 65, np.array([]), 100, 30)
    ...
    cv2.putText(
        image, text2, (halfx - 75, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2
    )
    ...
    cv2.putText(
        image, text3, (x - 150, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2
    )
